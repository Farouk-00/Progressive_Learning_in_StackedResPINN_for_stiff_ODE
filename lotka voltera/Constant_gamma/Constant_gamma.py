...
class StackedResPINN:
    def __init__(self, model, optimizer_primal, optimizer_dual, lr_scheduler=None, patience=100, patience_gamma=1000, N_collocation=1000, N_resample=50, N_dual=10, gammas_init=[], parameters_init=[1.1, 0.4, 0.4, 0.1], logger=None, T_max=2.0):
        self.model = model
    ...
    def update_stiffness_indicators(self, t):
        ...
        u2 = tf.squeeze(tf.stack(u2, axis=1), axis=2)

        #compute sigma
        sigma = (self.alpha - self.beta*u2 + self.delta*u1 - self.gamma) / 2.0
